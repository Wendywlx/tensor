...
void read_raw_data(std::istream &istr, _T &container) {
	...
	istr.read(reinterpret_cast<char *>(container.data()), container.size() * container.element_size());
}
...
void write_raw_data(std::ostream &ostr, const _T &container) {
	...
	ostr.write(reinterpret_cast<const char *>(container.data()), container.size() * container.element_size());
}
